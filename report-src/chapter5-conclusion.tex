...
\todo{move the following paragraph to chapter 5 - conclusions}
These results confirm that the ConvCNP, equipped with the elevation MLP and topographic features, can produce spatially coherent downscaled temperature fields at ${\sim}$1\,km resolution from ${\sim}$11\,km reanalysis input. The subsequent sections evaluate this capability quantitatively across the full holdout dataset.
...
\quad \texttt{2014-30e-5f}   & 1.31 & 1.71 & $-$0.05 & 1.21 & 0.524 \\
\midrule
\bottomrule
\end{tabular}
\end{table}
...
Despite this overconfidence, the model's predicted uncertainties remain informative: the correlation between predicted $\sigma$ and absolute error is 0.74 for the ten-year model, meaning that the uncertainty estimates reliably rank predictions by quality even though their absolute magnitudes are too small.
...
\quad \texttt{-ablation-seasonal-all}\textsuperscript{$\ddagger$}
...
\textsuperscript{$\ddagger$}\small These two ablations produced bit-identical metrics across all evaluation categories, despite differing in whether seasonal features are present in the CNN input. Interestingly, both of them present a higher skill than the base model. This unexpected result is examined in the ablation analysis (Section~\ref{sec:ablation}).
\end{flushleft}
\end{table}
